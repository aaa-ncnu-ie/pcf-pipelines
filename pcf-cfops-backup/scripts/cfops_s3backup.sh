...
echo "Logging out any logged in users from opsman ui so backup can proceed."
echo ""
...
curl "https://${OPS_MANAGER_HOSTNAME}/api/v0/sessions" -d ' ' -X DELETE -H "Authorization: Bearer ${CFOPS_ADMIN_TOKEN}" -H "Content-Type: application/x-www-form-urlencoded" --insecure -vv > /dev/null 2>&1

# for debugging purposes, check which tiles are available for cfops in the image
cfops version
echo "Executing cfops command for $TARGET_TILE"

# create backup file for the targeted tile and stores it in the output directory

if [ $TARGET_TILE == "ops-manager" ]
then
  cfops backup \
...
# fi
# Delete backup files from worker.
#cd $BUILD_DIR
